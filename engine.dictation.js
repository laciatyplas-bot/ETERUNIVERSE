...
class EterniverseVoiceDictation {
  ...
  init() {
    ...
    navigator.mediaDevices.getUserMedia({ audio: true }).then(() => {
      ...
      this.disableButtons();
      this.app?.setStatus?.('Brak pozwolenia na mikrofon. Udziel w ustawieniach przeglądarki.', 10000);
    });
  }
  ...
  setupRecognition() {
    ...
    this.recognition.onresult = (event) => {
      ...
      // Finalny tekst
      if (final) {
        const textarea = document.getElementById('bookContent');  // Integracja z modalem
        if (textarea) {
          ...
          console.warn('Brak textarea do dyktowania');
          this.app?.setStatus?.('Brak aktywnego edytora do dyktowania', 5000);
        }
